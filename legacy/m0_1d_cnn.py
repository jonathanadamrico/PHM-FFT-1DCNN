...
load_model = True
fs = 64000
anomaly_component = sys.argv[1] #"motor"
print(anomaly_component)
SEED = 123
...
def fft_preprocessing(df_, fs, test=False):
    ...
    for col in df.columns:
        ...
        for i in range(int(len(df)/fs)):  
            ...
            fft_abs = np.abs(fft_result[:len(fft_result)//2])
...
def get_test_features(components, sample_n, n_datapoints, final_stage=False):
    ...
    #for col in df.columns:
    #    if col in ("CH7","CH8","CH9"):
    #        continue
    #    X = df[col].copy()
    #    df[col] = filter_high_freqs(X, n_datapoints, cutoff_freq=16000)
...
if quick_load:
    X_train = np.load(f"Quick imports/X_train_{anomaly_component}.npy")
    y_train = np.load(f"Quick imports/y_train_{anomaly_component}.npy")
    X_val = np.load(f"Quick imports/X_val_{anomaly_component}.npy")
    y_val = np.load(f"Quick imports/y_val_{anomaly_component}.npy")
    ...
    for n_fault_type in tqdm(range(0,17)):
        ...
        if fault_type in component_fault[anomaly_component]:
            continue
        print(f"normal: {fault_type}")
    ...
    for folder_name in tqdm(final_train_labels):
        if component_to_fault[anomaly_component] in folder_name:
            print(f"normal: {folder_name}")
            ...
            for n_sample in range(1,total_samples+1):
                ...
                normal_temp = get_features(components, fault_type=None, sample_n=sample_n, n_datapoints=fs, folder_name=folder_name)
                if len(normal_temp) == 0:
                        continue
                if type(normal_arr) == type(None):
                    normal_arr = normal_temp.copy()
    ...
    # For anomalous types
    anomaly_arr = None
    for fault_type in tqdm(component_fault[anomaly_component]):
        print(f"anomaly: {fault_type}")
        for n_sample in range(1,4):
            sample_n = f"Sample{n_sample}"
    ...
    np.save(f"Quick imports/X_train_{anomaly_component}.npy", X_train)
    np.save(f"Quick imports/y_train_{anomaly_component}.npy", y_train)
    np.save(f"Quick imports/X_val_{anomaly_component}.npy", X_val)
    np.save(f"Quick imports/y_val_{anomaly_component}.npy", y_val)
...
def evaluate_model(trainX, trainy, testX, testy):
    ...
    #lr_scheduler = ExponentialDecay(initial_learning_rate=0.1, decay_steps=10000, decay_rate=0.01)
    #sgd = SGD(learning_rate=lr_scheduler, momentum=0.8, nesterov=False)
...
def evaluate_model(trainX, trainy, testX, testy):
    ...
    if load_model:
        model.load_state_dict(torch.load('saved_models'+'/'+f'1dcnn_model_{anomaly_component}.pth'))
    else:
        # Training loop
        for epoch in tqdm(range(epochs)):
            ...
            if early_stopping.early_stop:
                ...
                break
        # save model
        torch.save(model.state_dict(), 'saved_models'+'/'+f'1dcnn_model_{anomaly_component}.pth')
    ...
    with torch.no_grad():
        ...
        print(f"{anomaly_component} | Accuracy: {accuracy:.4} | Precision: {precision:.4} | Recall: {recall:.4} | F1: {f1:.4} | Z:{z_metric:.4}")
...
df = pd.DataFrame({"sample":test_labels["Sample Number"],
                   f"{anomaly_component}":predictions_prelim, #[:,0]# if with scaling
                   "true label":test_labels["FaultCode"]
                   })

df.to_csv(f"preds_1dcnn/preds_{anomaly_component}_1dcnn.csv", index=False)

# Calculate test performance
pred = np.round(predictions_prelim)
true = []
for i in range(len(test_labels)):
    value = int(test_labels.loc[i, "FaultCode"] in component_fault[anomaly_component])
    true.append(value)
...
final_df = pd.DataFrame({"sample":samples,
                   f"{anomaly_component}":predictions_final,
                   })

final_df.to_csv(f"preds_1dcnn_final/preds_{anomaly_component}_1dcnn.csv", index=False)
...
# June 06, 2024: Erroneous code was found which swapped the labels of the data for these anomaly_component models.
